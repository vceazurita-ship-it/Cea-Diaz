 ...
 *  · `fuerza`   — con el dedo puesto: la barra de potencia corre.
 *  · `punteria` — soltada la fuerza, la segunda barra cruza la diana.
 ...
 */
type Step = 'apuntar' | 'fuerza' | 'punteria' | 'corte' | 'vuelo' | 'visto';
...
/** Lo que dura la barra de puntería, en tanto por uno de la de fuerza. */
const PUNTERIA_RAPIDA = 0.52;
...
function TurnoTiro({
  ...
  /** El cartel de turno, que se quita solo y no estorba para apuntar. */
  const [cartel, setCartel] = useState(true);
  useEffect(() => {
    const t = window.setTimeout(() => setCartel(false), 1100);
  ...
  /* ---------------------------------------------------- la barra de puntería */

  /**
   * La segunda barra: más corta y más rápida, y con la diana en el centro.
   *
   * Es la que convierte el penalti en un tiro y no en una elección de casilla.
   * Pararla en la diana manda el balón justo adonde apunta la mira; pararla a
   * un lado lo abre a ese lado, que es el efecto de toda la vida. Y como el
   * abanico se estrecha con la precisión de la carta, elegir a Oliver o al
   * Tigre deja por fin de ser un adorno.
   */
  const aimFrame = useRef<number>();
  const aimed = useRef(50);
  const [aimBar, setAimBar] = useState(50);

  useEffect(() => {
    if (step !== 'punteria') return undefined;

    const started = performance.now();
    const sweep = SHOT_TYPES[kind].sweep * PUNTERIA_RAPIDA;
    ...
    const tick = (now: number) => {
      ...
      aimed.current = value;
      setAimBar(value);
      aimFrame.current = requestAnimationFrame(tick);
    };

    aimFrame.current = requestAnimationFrame(tick);
    return () => {
      if (aimFrame.current !== undefined) cancelAnimationFrame(aimFrame.current);
    };
  }, [step, kind]);
  ...
  /** Soltar la fuerza no chuta: la deja clavada y abre la puntería. */
  ...
  const fire = useCallback(() => {
    if (step !== 'punteria' || shot.current) return;
    shot.current = true;
    if (aimFrame.current !== undefined) cancelAnimationFrame(aimFrame.current);

    // La puntería, de -1 a 1: cero es clavada y las puntas son el abanico
    // entero abierto a un lado o a otro.
    const accuracy = (aimed.current - 50) / 50;
  ...
  const live = useRef({ step, charge, fire, lockPower });
  live.current = { step, charge, fire, lockPower };
  ...
  useEffect(() => {
    ...
    const down = (event: KeyboardEvent) => {
      ...
      if (now !== 'apuntar' && now !== 'fuerza' && now !== 'punteria') return;
      ...
      if (event.key === ' ' || event.key === 'Enter') {
        ...
        // La misma tecla hace las dos cosas por turnos: se mantiene para la
        // fuerza y se vuelve a pulsar para clavar la puntería.
        if (now === 'punteria') live.current.fire();
        else live.current.charge();
      }
    };
  ...
  const aiming = step === 'apuntar' || step === 'fuerza' || step === 'punteria';
  const zone = power < low ? 'flojo' : power > high ? 'pasado' : 'buena';
  const desvio = Math.abs(aimBar - 50);
  const tino = desvio <= 7 ? 'clavada' : desvio <= 20 ? 'cerca' : 'lejos';
  const pending = step === 'corte' || step === 'vuelo';

  // Mientras el balón va de camino el marcador sigue como estaba: el tiro ya
    ...
    <div className={`${manga.variable} mx-auto w-full max-w-2xl space-y-3`}>
      ...
      {aiming && (
        <div className="space-y-2.5">
          ...
          {/* Los tres tiempos del tiro comparten **el mismo botón**: quien
              ...
              salto, y además dice en cada momento qué toca: mantenerlo para la
              fuerza, soltarlo en la franja verde y volver a tocarlo para clavar
              la puntería. */}
          <button
            type="button"
            onPointerDown={(event) => {
              ...
              if (step === 'punteria') fire();
              else charge();
            }}
            onPointerUp={lockPower}
            onPointerCancel={lockPower}
            ...
            className={`relative flex min-h-[4rem] w-full touch-none select-none items-center justify-center gap-2 overflow-hidden
              ...
              ${
                step === 'apuntar'
                  ...
                  : step === 'punteria'
                    ? tino === 'clavada'
                      ? 'animate-latido bg-gradient-to-b from-sky-300 to-sky-500 text-[#0b1f14]'
                      : tino === 'cerca'
                        ? 'bg-gradient-to-b from-sky-400 to-indigo-600 text-white'
                        : 'bg-gradient-to-b from-indigo-500 to-violet-700 text-white'
                    : zone === 'buena'
          ...
          <p className="text-center text-[11px] leading-snug t-3">
            {step === 'apuntar' ? (
              ...
              'Segunda barra: párala en la diana y el balón va justo donde apuntas. Si la paras a un lado, se abre a ese lado.'
            ) : (
...
function Escena({
  ...
  power: number;
  band: [number, number];
  /** Por dónde va la segunda barra, la de la puntería. */
  aimBar: number;
  /** Lo que abre el balón este jugador, según su precisión. */
  abanico: number;
  ...
  const flight = (slow: number): Animation | null => {
    if (!route || !ballRef.current || !fired) return null;
    return volar(
      ballRef.current,
      route,
      SHOT_TYPES[fired.kind].flight,
      fired.kind === 'parabola' ? 'ease-in-out' : 'cubic-bezier(0.3, 0.6, 0.4, 1)',
      slow,
    );
  };
  ...
  // Y lo que se sabe del efecto mientras cruza la segunda barra: adónde se
  // abriría el balón si se soltara ahora. Se pinta en vivo porque es la
  // única manera de que se entienda para qué sirve la barra.
  const aiming2 = step === 'punteria';
  const tino = (aimBar - 50) / 50;
  const curve = aiming2 ? enEscena({ x: Math.max(-8, Math.min(108, aim.x + tino * ABANICO * abanico)), y: aim.y }) : null;
  const clavada = Math.abs(tino) <= 0.14;
  ...
  return (
    ...
    >
      ...
      {/* El efecto que se está cogiendo: la raya que va de la mira al sitio
          ...
          Es lo que hace que la barra se entienda sin explicarla. */}
      {curve && (
        <>
          <svg aria-hidden className="pointer-events-none absolute inset-0 z-20 h-full w-full" viewBox="0 0 100 100" preserveAspectRatio="none">
            <line
          ...
          <div
            aria-hidden
            className="pointer-events-none absolute z-20 h-5 w-5 -translate-x-1/2 -translate-y-1/2 rounded-full border-2"
            style={{
              left: `${curve.x}%`,
              top: `${curve.y}%`,
              borderColor: clavada ? '#38bdf8' : '#818cf8',
              backgroundColor: clavada ? 'rgba(56,189,248,0.3)' : 'rgba(129,140,248,0.2)',
            }}
          />
        </>
      )}
      ...
      {aiming && (
        ...
        >
          <div className="ml-auto w-[60%] min-w-[10rem]">
          ...
          {/* Y debajo, la de la puntería: la misma barra con la diana en el
              centro en vez de una franja al final. Sale sólo cuando toca, y
              deja la de potencia a la vista, ya clavada, para que se vea que
              el tiro se está montando por partes. */}
          {aiming2 && (
            <div className="mt-1.5">
              <div className="mb-0.5 flex items-end justify-end gap-2 px-0.5 text-[clamp(8px,2.2vw,11px)] font-black uppercase tracking-[0.14em] text-white [text-shadow:0_1px_2px_rgba(0,0,0,0.8)]">
                <span>Puntería</span>
                <span style={{ color: ring }}>{clavada ? '¡Clavada!' : tino < 0 ? '◀ se abre' : 'se abre ▶'}</span>
              </div>
              <div className="relative h-[clamp(12px,3.4vw,18px)] -skew-x-12 overflow-hidden rounded-[3px] border-2 border-[#0b1220] bg-[#0b1220]/80 shadow-[0_2px_6px_rgba(0,0,0,0.5)]">
                <div className="absolute inset-0 bg-gradient-to-r from-indigo-600 via-sky-300 to-indigo-600 opacity-60" />
                {/* La diana: la franja estrecha del centro, la que hay que clavar. */}
                <div className="absolute inset-y-0 left-[43%] w-[14%] border-x-2 border-sky-200 bg-sky-300/50" />
                <div
                  className="absolute inset-y-[-2px] w-[3px] bg-white shadow-[0_0_6px_white]"
                  style={{ left: `calc(${aimBar}% - 1px)` }}
                />
              </div>
            </div>
          )}
      ...
      {(step === 'fuerza' || step === 'punteria') && (
        <div className="pointer-events-none absolute right-[3%] top-[3%] z-20 w-[19%] -rotate-3 animate-pop">
      ...
      {/* El cartel de turno: medio segundo diciendo qué toca. No estorba para
          apuntar —no recibe toques— y es lo que evita que la tanda cambie de
          manos sin avisar. */}
      {cartel && step === 'apuntar' && (
        <Cartel
