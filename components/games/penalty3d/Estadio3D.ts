...
import { clamp01, crearMuneco, lerp, posar, suave, type Muneco, type Pose, type Ropa } from '@/components/games/penalty3d/muneco';
...
export class Estadio3D {
  ...
  /** Un muñeco de la serie: el de `muneco.ts`, con sus piezas apuntadas para soltarlas. */
  private muneco(ropa: Ropa): Muneco {
    return crearMuneco({ geo: (g) => this.geo(g), mat: (m) => this.mat(m) }, ropa);
  }
  ...
  private posar(m: Muneco, pose: Pose, t: number) {
    posar(m, pose, t);
  }
